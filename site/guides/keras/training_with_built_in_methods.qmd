...
- `update_state(self, y_true, y_pred, sample_weight = NULL)`, which uses the targets
...
class CategoricalTRUEPositives(keras$metrics$Metric):
    initialize <- function(name = "categorical_true_positives", **kwargs) {    }
        super$initialize(name = name, **kwargs)
...
```{r}
inputs <- layer_input(shape = (784,), name = "digits")
x1 <- layer_dense(64, activation = "relu", name = "dense_1")(inputs)
x2 <- layer_dense(64, activation = "relu", name = "dense_2")(x1)
outputs <- layer_dense(10, name = "predictions")(x2)
model <- keras_model(inputs = inputs, outputs = outputs)
...
logits <- layer_dense(10)(inputs)
predictions <- LogisticEndpoint(name = "predictions")(logits, targets)

model <- keras_model(inputs = [inputs, targets], outputs = predictions)
...
dict(zip_lists(model$metrics_names, result))
```
...
sample_weight <- np$ones(shape = (length(y_train),))
...
image_input <- layer_input(shape = (32, 32, 3), name = "img_input")
timeseries_input <- layer_input(shape = (NULL, 10), name = "ts_input")

x1 <- layer_conv_2d(3, 3)(image_input)
x1 <- layer_global_max_pooling_2d()(x1)

x2 <- layer_conv_1d(3, 3)(timeseries_input)
x2 <- layer_global_max_pooling_1d()(x2)

x <- layers$concatenate([x1, x2])

score_output <- layer_dense(1, name = "score_output")(x)
class_output <- layer_dense(5, name = "class_output")(x)
...
model %>% compile(
    optimizer <- keras$optimizers$RMSprop(1e-3),
    loss <- [NULL, keras$losses$CategoricalCrossentropy()],
)
...
model %>% compile(
    optimizer <- keras$optimizers$RMSprop(1e-3),
    loss <- list("class_output": keras$losses$CategoricalCrossentropy()),
)
...
callbacks <- [
    ...
    keras$callbacks$ModelCheckpoint(
        filepath <- checkpoint_dir + "/ckpt-loss = list(loss:.2f)", save_freq = 100
    )
]
