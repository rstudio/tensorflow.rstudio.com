...
raw_train_ds_iterator <- as_iterator(raw_train_ds)
batch <- iter_next(raw_train_ds_iterator)
text_batch <- batch[[1]]
label_batch <- batch[[2]]
for (i in 1:3) {
  print(text_batch[i])
...
text_ds <- raw_train_ds %>%
  dataset_map(function(x, y) { return(x) } )
# Let's call `adapt`:
...
```{r}
x <- keras::layer_input(shape = tuple(1L), dtype = tf$string, name = 'text') %>% 
  vectorize_layer() %>% 
...
vectorize_text <- function(text, label) {
  ...
  return(tuple(vectorize_layer(text), label))
}
...
```{r}
inputs <- layer_input(shape = tuple(NULL), dtype = "int64")
...
```{r}
# A string input
inputs <- layer_input(shape = tuple(1L), dtype = "string")
