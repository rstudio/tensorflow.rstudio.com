...
> TensorFlow Estimators has not received updates by the TensorFlow team in a long time, and it's no longer recommended. You may find bugs when using `tfestimators` with recent versions of TensorFlow (>= 1.10).
...
You can also write fully custom input functions that draw data from arbitrary data sources. See the [input functions](../input_functions/) article for additional details.
...
See the article on [feature columns](../feature_columns/) for additional details.
