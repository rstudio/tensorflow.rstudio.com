...
serializables (see https://www.tensorflow.org/api_docs/python/tf/keras/utils/register_keras_serializablelist("tf$keras$utils$register_keras_serializable")
for more details).
...
in this example, we should see something like `list(c("string", "to", "split"), c("another", "string", "to", "split"))`. This makes the callable
