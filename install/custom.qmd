...
Note that if you use the argument `required = FALSE`, you can include multiple calls to the `use_` functions and all
provided locations will be tried in the order they were specified. For example:
```{r}
library(tensorflow)
...
`install_keras()` function installs these dependencies
