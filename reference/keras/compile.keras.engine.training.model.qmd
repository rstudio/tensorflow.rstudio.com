...
| run_eagerly | Bool. Defaults to `FALSE`. If `TRUE`, this Model's logic will not be wrapped in a `tf.function`. Recommended to leave this as `NULL`<br>unless your Model cannot be run inside a `tf.function`. `run_eagerly=True`<br>is not supported when using `tf.distribute.experimental.ParameterServerStrategy`. If the model's logic uses tensors in R control flow expressions like `if` and `for`, the model is still traceable with `tf.function`, but you will have to enter a `tfautograph::autograph({})` directly. |
