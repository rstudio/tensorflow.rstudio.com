...
If you don't already have one, create a Python installation that you
will use with *Tensorflow for R*. The easiest way to do this is like
this:

```{r}
library(reticulate)
virtualenv_create(envname = "r-reticulate", python = install_python())
```
...
The default TensorFlow package will run on computations on a GPU if one
is available. and can be configured to run on either CPUs or GPUs. If
you do not have a GPU then you can explicitly specify that a cpu-only
build of tensorflow be installed like this:

```{r}
install_tensorflow(version = "cpu")
```

If you have an NVIDIA® GPU on your machine you want Tensorflow to use,
you must also ensure that the correct CUDA and cuDNN versions are
installed.
...
-   **TensorFlow with GPU support**. TensorFlow programs typically run
    ...
    system has a Nmeeting the prerequisites shown below and you need to
    run performance-critical applications, you should ultimately install
    this version.
...
Note that this could be a full `major.minor.patch` version
specification, or just major and minor versions.
...
install_tensorflow(version = "nightly")  # gpu version
```
